...
describe('+ FilesController', () => {
  ...
  const emptyDatabaseCollections = (callback) => {
    Promise.all([dbClient.usersCollection(), dbClient.filesCollection()])
      .then(([usersCollection, filesCollection]) => {
        ...
        if (callback) callback(connectErr);
      });
  };
  ...
  describe('+ GET: /files/:id', () => {
    ...
    it('+ Fails for a non-existent user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get('/files/444555666')
          .set('X-Token', 'raboof')
          .expect(401)
          .end((requestErr, res) => {
            if (requestErr) {
    ...
    it('+ Succeeds if file is linked to user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get(`/files/${mockFiles[0].id}`)
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
  ...
  describe('+ GET: /files', () => {
    ...
    it('+ Fetches first page with no page query and parentId', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get(`/files?parentId=${mockFiles[1].id}`)
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
    ...
    it('+ Returns empty list for a parentId of a file', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get(`/files?parentId=${mockFiles[0].id}`)
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
    ...
    it('+ Returns empty list for unknown parentId', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get('/files?parentId=34556ea6727277193884848e')
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
  ...
  describe('+ PUT: /files/:id/publish', () => {
    ...
    it('+ Fails for a non-existent user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).put('/files/444555666/publish')
          .set('X-Token', 'raboof')
          .expect(401)
          .end((requestErr, res) => {
            if (requestErr) {
    ...
    it('+ Succeeds if file is linked to user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).put(`/files/${mockFiles[0].id}/publish`)
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
  ...
  describe('+ PUT: /files/:id/unpublish', () => {
    ...
    it('+ Fails for a non-existent user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).put('/files/444555666/unpublish')
          .set('X-Token', 'raboof')
          .expect(401)
          .end((requestErr, res) => {
            if (requestErr) {
    ...
    it('+ Succeeds if file is linked to user', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).put(`/files/${mockFiles[0].id}/unpublish`)
          .set('X-Token', token)
          .expect(200)
          .end((requestErr, res) => {
            if (requestErr) {
  ...
  describe('+ GET: /files/:id/data', () => {
    ...
    it('+ Fails if the file is not public and not requested by the owner', () => new Promise((done) => {
      request(app).get(`/files/${mockFiles[0].id}/data`)
        .expect(404)
        .end((err, res) => {
          if (err) {
            return done(err);
    ...
    it('+ Fails if the file is a folder', function () {
      return new Promise((done) => {
        this.timeout(5000);
        request(app).get(`/files/${mockFiles[1].id}/data`)
          .expect(400)
          .end((requestErr, res) => {
            if (requestErr) {
              console.error(requestErr);
    ...
    it('+ Succeeds if the file is not public but requested by the owner [alt]', () => new Promise((done) => {
      request(app).get(`/files/${mockFiles[2].id}/data`)
        .set('X-Token', token)
        .expect(200)
        .end((err, res) => {
          if (err) {
